...
internal static class RefreshRightPage
{
    ...
    private static class RefreshRightPagePatch
    {
        ...
        private static void Postfix(Panel_FirstAid __instance)
        {
            ...
            switch (__instance.m_SelectedAffButton.m_AfflictionType)
            {
                ...
                case AfflictionType.Generic: // Custom affliction.
                    ...
                    {
                        if (affliction.HasRemedies() && interfaceRemedies != null)
                        {
                        ...
                        {
                            remedySprites = System.Array.Empty<string>();
                            remedyNumRequired = System.Array.Empty<int>();
                            remedyComplete = System.Array.Empty<bool>();
                            altRemedySprites = null;
                            altRemedyNumRequired = null;
                            altRemedyComplete = null;
                        }
                        ...
                        __instance.SetItemsNeeded(remedySprites, remedyComplete, remedyNumRequired, altRemedySprites, altRemedyComplete, altRemedyNumRequired, ItemLiquidVolume.Zero, interfaceRestTreatment?.RestAmountRemaining ?? 0f, interfaceRestTreatment?.RestAmountRequired ?? 0f);
                        SetDoseTreatment(__instance, interfaceDoseTreatment);
                    }
                    else
                    {
                        __instance.m_RightPageObject.SetActive(false);

                        if (interfaceSpecialTreatment != null)
                        {
                            __instance.m_LabelAfflictionDescriptionNoRest.text = string.Empty;
                            __instance.m_LabelAfflictionDescription.text = string.Empty;
                            __instance.m_LabelRiskDescription.text = string.Empty;
                            __instance.m_BuffWindow.SetActive(false);
                            __instance.m_LabelSpecialTreatment.text = interfaceSpecialTreatment.SpecialTreatmentText;
                        ...
                        else if (!string.IsNullOrEmpty(affliction.m_DescriptionNoHeal) && !affliction.HasBuff())
                        {
                            __instance.m_LabelAfflictionDescriptionNoRest.text = string.Empty;
                            __instance.m_LabelAfflictionDescription.text = string.Empty;
                            __instance.m_BuffWindow.SetActive(false);
                    ...
                    if (affliction.HasRisk() && riskPercentage != null) // Need to add another check in here to actually determine if the risk affliction has a timer or not.
                    {
        ...
        private static void SetDoseTreatment(Panel_FirstAid panel, IDoseTreatment? doseTreatment)
        {
            if (doseTreatment == null || doseTreatment.DosesRequired <= 0)
            {
                panel.m_MultipleDosesObject.SetActive(false);
                return;
            }

            panel.m_MultipleDosesObject.SetActive(true);
